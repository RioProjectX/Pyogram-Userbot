...
from riobot import HELP, LOGS, PM_AUTO_BAN, PM_MSG_COUNT, PM_UNAPPROVED, TEMP_SETTINGS
from riobot.modules.chat import is_muted
from rionify.core import edit, get_translation, reply, riobot, send_log
...
@riobot(outgoing=True, pattern='^.notifoff$')
...
@riobot(outgoing=True, pattern='^.notifon$')
def notifon(message):
    try:
...
@riobot(outgoing=True, pattern='^.approve$', compat=False)
...
@riobot(outgoing=True, pattern="^.disapprove$")
