...
from riobot import HELP, HEROKU_APPNAME, HEROKU_KEY
from rionify.core import edit, get_translation, reply_doc, sedenify, send_log


@riobot(pattern='^.(notheroku|ko)ta$')
def dyno(message):
    if not HEROKU_KEY:
...
@riobot(pattern='^.(restart|yb)$')
def _restart(message):
    return restart(message)


@riobot(pattern='^.d(restart|yb)$')
